...
cd $resultdir
filename=$(basename $(ls *.json | head -n 1))
...
cd $resultdir/json
python3 $basedir/utils/collect_results.py ../agg/ ${model}_${seqlens}

cd $basedir
echo "Results collected to"
find $resultdir/json/ -name "agg_${model}_${seqlens}*.json"